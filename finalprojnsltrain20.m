...
writeID = fopen('NSLKDDTrain20Altered','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
%features(:,length(newtable(1,:))-1)= newtable(:,length(newtable(1,:))); %the 42nd column of the features table is equal to the 43rd column of the NSL KDD set since the NSL KDD set adds an extra feature
...
writeID = fopen('NSLKDDTestAltered20','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
writeID = fopen('NSLKDDTest21Altered20','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
writeID = fopen('newcorrected20','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
