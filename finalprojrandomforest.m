...
In particular, we use the Proj. 2 parameters of 10/1, 50/2, 500/8, 1000/16 trees/features sampled. 

Additionally, we measure the importance of features, choose the most important features
and then based on this we preprocess the data again, and vary both variables as done above to see if we can
 find a combination where we have a lot fewer features but still accurate
 classification
%}

fileID = fopen('NSLKDDTrain+.csv');
writeID = fopen('NSLKDDTrainAlteredfull','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
model = classRF_train(features,labels, 100,2);
...
writeID = fopen('NSLKDDTestAlteredfull','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
writeID = fopen('NSLKDDTest21Alteredfull','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
%we next process the original training data per our new model
fileID = fopen('corrected');
writeID = fopen('newcorrectedfull','wt');
i=1;
tline = fgetl(fileID);
A=tline;
while ischar(tline)
...
features_sparse = sparse(features);
fprintf('Statistics for testing using the previously trained model (unbiased) on the original KDD 99 testing data: ');
[predicted_label, accuracy,prob_estimates] = predict(labels, features_sparse, model);
fprintf('Statistics for testing using the previously trained model (biased) on the original KDD 99 testing data: ');
[predicted_label1, accuracy1,prob_estimates1] = predict(labels, features_sparse, model1);
